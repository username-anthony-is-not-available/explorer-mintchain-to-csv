...
def test_cli_json_output(responses):
    ...
    responses.add(
        responses.GET,
        f"{MINTCHAIN_API_URL}?module=account&action=txlist&address={TEST_WALLET_ADDRESS}&startblock=0&endblock=99999999&offset=10000&sort=asc&page=1",
        json={
            "result": [
                {
                    "blockNumber": "1",
    ...
    responses.add(
        responses.GET,
        f"{MINTCHAIN_API_URL}?module=account&action=tokentx&address={TEST_WALLET_ADDRESS}&startblock=0&endblock=99999999&offset=10000&sort=asc&page=1",
        json={"result": []},
        status=200,
    )
    responses.add(
        responses.GET,
        f"{MINTCHAIN_API_URL}?module=account&action=txlistinternal&address={TEST_WALLET_ADDRESS}&startblock=0&endblock=99999999&offset=10000&sort=asc&page=1",
        json={"result": []},
        status=200,
    )
