...
def extract_transaction_data(
    ...
    for trx in transaction_data:
        ...
        if isinstance(trx, RawTransaction):
            ...
            data['Description'] = f"NFT transfer: {trx.tokenName} ({trx.tokenSymbol}) #{trx.tokenID}"
            if is_sender:
                data['Sent Amount'] = "1"
            ...
            data['Description'] = f"ERC-1155 transfer: {trx.tokenName} ({trx.tokenSymbol}) #{trx.tokenID}"
            if is_sender:
